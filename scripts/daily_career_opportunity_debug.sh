...
# Set AnySearch API key and endpoint
export ANYSEARCH_API_KEY="as_sk_b1df60bd2ea90b833fac22494eb6da0c"
export ENDPOINT="https://api.anysearch.com/mcp"
...
MAX_PER_QUERY=2
TMP_RESULTS=$(mktemp)        # will hold title|url lines
TMP_PROCESSED=$(mktemp)      # will hold processed lines for sorting
...
if [[ ! -s "$TMP_RESULTS" ]]; then
    ...
    rm -f "$TMP_RESULTS" "$TMP_PROCESSED"
...
while IFS='|' read -r title url; do
    ...
    # Determine opportunity direction (heuristic from title) using case
    case "$title" in
        *outbound*|*international*|*global*|*expansion*|*海外*|*出海*)
            direction="国际市场/出海机会"
            ;;
        *agent*|*multi[-]agent*|*platform*)
            direction="新AI平台/Agent平台"
            ;;
        *workflow*|*automation*|*SOP*)
            direction="AI工作流/自动化工具"
            ;;
        *IDE*|*editor*|*code*)
            direction="AI IDE/编辑器"
            ;;
        *browser*|*search*)
            direction="AI浏览器/AI搜索入口"
            ;;
        *distribution*|*marketplace*|*app[[:space:]]store*|*plugin*)
            direction="应用分发渠道/插件生态"
            ;;
        *hardware*|*robot*|*robotics*)
            direction="AI硬件/机器人产品"
            ;;
        *BD*|*partnership*|*channel*|*GTM*)
            direction="商务拓展/渠道合作机会"
            ;;
        *user[[:space:]]entry*|*entry[[:space:]]point*|*onboarding*)
            direction="新用户入口/交互界面"
            ;;
        *world[[:space:]]model*|*physical[[:space:]]AI*|*agent[[:space:]]OS*)
            direction="World Model/Physical AI/Agent OS"
            ;;
        *)
            direction="AI产品/技术更新"
            ;;
    ...
    case "$direction" in
        "国际市场/出海机会")
            why="该公司正在布局海外市场，为国际业务BD提供切入点。"
            ;;
        "新AI平台/Agent平台")
            why="新平台提供集成机会，个人开发者可上架作品或获取合作。"
            ;;
        "AI工作流/自动化工具")
            why="工作流工具提升开发效率，可作为技能点或远程岗位依据。"
            ;;
        "AI IDE/编辑器")
            why="新IDE降低开发门槛，适合个人开发者快速原型。"
            ;;
        "AI浏览器/AI搜索入口")
            why="新入口增加流量获取渠道，有利于产品推广。"
            ;;
        "应用分发渠道/插件生态")
            why="分发渠道帮助作品触达用户，可考虑上架或合作。"
            ;;
        "AI硬件/机器人产品")
            why="硬件产品伴随软件生态，提供硬件+软件结合的岗位。"
            ;;
        "商务拓展/渠道合作机会")
            why="明确BD需求，可申请相关实习或合作项目。"
            ;;
        "新用户入口/交互界面")
            why="新交互形式是产品经理方向的切入点。"
            ;;
        "World Model/Physical AI/Agent OS")
            why="长期趋势，需关注其开发者社区和应用场景。"
            ;;
        *)
            why="具备技术或市场机遇，值得持续关注。"
            ;;
    esac
    ...
    case "$direction" in
        "国际市场/出海机会")
            action="关注该公司的国际岗位招聘或BD合作公告。"
            ;;
        "新AI平台/Agent平台")
            action="注册开发者账号，查看文档和上架流程。"
            ;;
        "AI工作流/自动化工具")
            action="试用该工具，制作简历中的项目案例。"
            ;;
        "AI IDE/编辑器")
            action="下载体验，评估其对个人开发的助力。"
            ;;
        "AI浏览器/AI搜索入口")
            action="使用其搜索功能，观察收录情况和SEO机会。"
            ;;
        "应用分发渠道/插件生态")
            action="了解上架要求，尝试提交一个小作品。"
            ;;
        "AI硬件/机器人产品")
            action="关注其开发者套件或硬件兼容性信息。"
            ;;
        "商务拓展/渠道合作机会")
            action="投递实习或岗位，准备相关市场分析材料。"
            ;;
        "新用户入口/交互界面")
            action="分析其用户流程，撰写可用性改进建议。"
            ;;
        "World Model/Physical AI/Agent OS")
            action="追踪其技术博客，学习相关概念和潜在应用。"
            ;;
        *)
            action="保持关注，定期查看更新。"
            ;;
    ...
    case "$direction" in
        "国际市场/出海机会"|"新AI平台/Agent平台"|"应用分发渠道/插件生态"|"商务拓展/渠道合作机会"|"新用户入口/交互界面")
            importance="高"
            ;;
        "AI工作流/自动化工具"|"AI IDE/编辑器"|"AI浏览器/AI搜索入口"|"AI硬件/机器人产品")
            importance="中"
            ;;
        *)
            importance="低"
...
done < "$TMP_RESULTS" > "$TMP_PROCESSED"
...
awk -F'\t' '{
    ...
    printf "%d\t%s\t%s\n", weight, $1, $0;
}' "$TMP_PROCESSED" | sort -t$'\t' -k1,1n -k2,2 | cut -f3- > "$TMP_SORTED"
...
echo "=== Daily Career Opportunity Assessment finished at $(date) ===" >&2
# Cleanup
rm -f "$TMP_RESULTS" "$TMP_PROCESSED" "$TMP_SORTED"
