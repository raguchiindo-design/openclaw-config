...
- [ ] Monitor 农业科技周报深度版 cron job (ID: 7ac9556c-f472-486f-9d47-71b7133e1aa1) - 2026-05-19 heartbeat复查：当前 jobs.json payload model 已是 openai-codex/gpt-5.5；cron list 仍显示 error 是上次运行残留状态（最近一次错误为旧模型 openrouter/xiaomi/mimo-v2-flash 被 allowlist 拒绝）。下次计划约 3 天后运行，届时再确认是否恢复。
