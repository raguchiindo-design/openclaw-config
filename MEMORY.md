...
### 数据查询偏好（2026-03-03新增）
- **默认联网对比**：任何指标或数据的最新状况，必须联网获取最新信息，而非仅依赖记忆中的
## 2026-06-13

### 每日AI机会雷达的要点与对小雪意义的质量保证
**时间**：2026年6月13日（北京时间）

为确保推送内容的“要点”和“对小雪的意义”具备实质价值，采取以下三项措施：

1. **明确评价维度**：在抓取标题后，先用规则判断是否涉及「新平台/agent、新界面、可集成/分发、开发工具/SDK/API、远程工作机会、未来信号」等六个维度，只保留命中≥1 维的条目，低质杂谈直接过滤。

2. **要点生成**：对每条文章，先取正文前 150 字（或摘要），用关键句抽取（如第一个完整的中文句子或包含动词-宾语结构的句子），再控制在 20 ± 3 字；若正文不足，则标题+副标题组合后再截取 20 字，确保信息密度高且无冗词。

3. **对小雪的意义**：基于同一评价维度，给出对应的个人开发者价值（如「可直接集成的 SDK」「降低训练成本的开源模型」「适合远程兼职的岗位」等），用一句话概括，避免泛谈；可预设模板（“为开发者提供 [X] ，助力 [Y] ”），填入具体技术或机会名称。

这样在脚本层面加入初步过滤、结构化抽取和模板填空，就能在不依赖人工审阅的前提下保持“要点”和“对小雪的意义”的实用质量.

### 每日AI机会雷达定时任务
**任务ID**：bada3c2e-de65-42f4-8032-1fb2143beed5
**执行时间**：每天 13:00 北京时间（Asia/Shanghai）
**任务内容**：运行脚本 `bash /home/ubuntu/.openclaw/workspace/scripts/ai_opportunity_radar.sh`，生成20条AI机会信息（标题+约20字要点+对小雪的意义+短链接），通过 Telegram announcement 发送给小雪（chat ID 7656385011）。
**未来一周推送时间**：
- 2026-06-13 13:00
- 2026-06-14 13:00
- 2026-06-15 13:00
...
**执行时间**：每天 15:00 北京时间（Asia/Shanghai）
**任务内容**：运行脚本 `bash /home/ubuntu/.openclaw/workspace/scripts/daily_career_opportunity.sh`，生成职业机会评估（机会方向、相关公司/产品、为什么值得关注、Cici可以做的小行动、重要程度），通过 Telegram announcement 发送给小雪（chat ID 7656385011）。
**未来一周推送时间**：
- 2026-06-13 15:00
- 2026-06-14 15:00
- 2026-06-15 15:00
